...
struct MacheteKernelTemplate {
  ...
  static Arguments create_arguments(
      ...
      std::optional<torch::stable::Tensor> const& maybe_g_scales,  // scale_KxN matrix
      std::optional<torch::stable::Tensor> const& maybe_g_zeros,   // scale_KxN matrix
      std::optional<int64_t> maybe_group_size,
      std::optional<torch::stable::Tensor> const& maybe_ch_scales,   // len N vector
      std::optional<torch::stable::Tensor> const& maybe_tok_scales)  // len M vector
  {
  ...
  static void run(Arguments const& args, void* workspace, cudaStream_t stream) {
    ...
    STD_TORCH_CHECK(status == cutlass::Status::kSuccess, "Machete kernel failed");
  }
};
