...
torch::stable::Tensor mm(torch::stable::Tensor const& A,
                         torch::stable::Tensor const& B, int64_t b_type_id,
...
torch::stable::Tensor prepack_B(
    ...
    torch::headeronly::ScalarType const& a_type, int64_t b_type_id,
    std::optional<torch::headeronly::ScalarType> const& maybe_group_scales_type) {
  ScalarType const b_type = ScalarType::from_id(b_type_id);
...
// use CompositeExplicitAutograd since supported_schedules has no tensor arguments
