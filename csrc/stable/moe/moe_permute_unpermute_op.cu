...
void moe_permute(
  ...
  torch::stable::Tensor copy_topk_ids =
      torch::stable::clone(topk_ids);  // copy topk_ids for preprocess
  ...
  if (align_block_size.has_value()) {
    torch::stable::Tensor align_expert_first_token_offset =
        torch::stable::new_zeros(expert_first_token_offset,
                                 expert_first_token_offset.sizes());
