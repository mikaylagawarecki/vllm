...
#include "stable/stable_dispatch_utils.h"
...
expert_map = torch::stable::empty(
        {0}, torch::headeronly::ScalarType::Int, std::nullopt, topk_ids.device());
  }
...
torch::stable::Tensor cumsum_buffer = torch::stable::empty(
          ...
          align_kernel<<<2, threads, shared_mem_size, stream>>>(
              ...
              experts_ids.mutable_data_ptr<int32_t>(),
              num_tokens_post_pad.mutable_data_ptr<int32_t>(),
              expert_map.mutable_data_ptr<int32_t>(), num_experts, padded_num_experts,
              experts_per_warp, block_size, topk_ids.numel(),
              cumsum_buffer.mutable_data_ptr<int32_t>(), sorted_token_ids.size(0),
          ...
          sort_kernel<<<gridDims, block_threads, 0, stream>>>(
              ...
              expert_map.mutable_data_ptr<int32_t>(),
...
void batched_moe_align_block_size(int64_t max_tokens_per_batch,
  ...
  batched_kernel::batched_moe_align_block_size_kernel<<<
      ...
      B, max_tokens_per_batch, block_size, batch_num_tokens.const_data_ptr<int32_t>(),
      sorted_ids.mutable_data_ptr<int32_t>(), batch_ids.mutable_data_ptr<int32_t>(),
      num_tokens_post_pad.mutable_data_ptr<int32_t>());
}
             ...
             torch::stable::Tensor& output)  // [num_tokens, hidden_size]
{
  ...
  torch::stable::accelerator::DeviceGuard device_guard(output.get_device_index());
  const cudaStream_t stream = get_current_cuda_stream(output.get_device_index());
...
void moe_lora_align_block_size(
    ...
    torch::stable::Tensor num_tokens_post_pad, torch::stable::Tensor adapter_enabled,
    torch::stable::Tensor lora_ids, std::optional<torch::stable::Tensor> maybe_expert_map) {
      ...
      topk_ids.scalar_type(), "moe_lora_align_sum_kernel", [&] {
        ...
        if (small_batch_expert_mode) {
          ...
          kernel<<<max_loras, blockDim, shared_mem, stream>>>(
              ...
              num_tokens_post_pad.mutable_data_ptr<int32_t>(),
              adapter_enabled.mutable_data_ptr<int32_t>(), lora_ids.mutable_data_ptr<int32_t>(),
          ...
          // cumsum buffer
          torch::stable::Tensor cumsum = torch::stable::empty(
              {max_loras * (num_experts + 1)},
              torch::headeronly::ScalarType::Int, std::nullopt, topk_ids.device());
          ...
          align_kernel<<<max_loras * 2, blockDim, shared_mem_size, stream>>>(
              ...
              expert_ids.mutable_data_ptr<int32_t>(), topk_num,
              num_tokens_post_pad.mutable_data_ptr<int32_t>(),
              adapter_enabled.mutable_data_ptr<int32_t>(), cumsum.mutable_data_ptr<int32_t>(),
              WARP_SIZE, padded_num_experts, lora_ids.mutable_data_ptr<int32_t>(),
              token_mask.mutable_data_ptr<int32_t>(), has_expert_map);
          ...
          sort_kernel<<<gridDims, block_threads, 0, stream>>>(
              topk_ids.const_data_ptr<scalar_t>(),
              sorted_token_ids.mutable_data_ptr<int32_t>(), cumsum.mutable_data_ptr<int32_t>(),
              expert_map.mutable_data_ptr<int32_t>(), topk_ids.numel(), num_experts,
              max_num_tokens_padded, topk_num, token_mask.mutable_data_ptr<int32_t>(),
